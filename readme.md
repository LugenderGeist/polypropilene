...
В проекте возможно определить нелинейные зависимости между параметрами и построить альтернативную тепловую карту - distance correlation matrix. Distance correlation — это мера, которая определяет любую зависимость между переменными (не только линейную). Она работает через сравнение попарных расстояний между точками: если расстояния между точками в X похожи на расстояния между соответствующими точками в Y — есть связь. Значение 0 означает независимость, 1 — функциональную зависимость. В отличие от корреляции Пирсона, видит синусы, параболы и другие нелинейности.

<div align="center">
  <img src="data/examples/distance_correlation_heatmap.png" alt="distance_correlation_heatmap.png" width="483">
</div>

После построения матрицы алгоритм пытается найти простейшие нелинейные зависимости между входными переменными и целевой. (На момент тестирования зависимости оказались сликшом сложными, чтобы отобразить их, поэтому примера нет)
## Фильтрация
...
Изменить гиперпараметры моделей можно в config.py для увеличения точности предсказаний. Модели можно обучить по очереди и сравнить результаты.  
...
Оптимизация осуществляется либо по n количеству наиболее значимых переменных, либо по всем входных величинам.  
...
Разработана функция генерации сценариев для работы строгой модели. Используется метод латинского гиперкуба. Генерация сценариев выполняется либо по n количеству наиболее важных параметров, либо по всем входным величинам.
Генерация сценариев выполняется либо по n количеству наиболее важных параметров, либо по всем входным величинам.
...
Весь проект возможно использовать для валидации строгой модели: создаются сценарии, выбранная модель машинного обучения строит свои предсказания на выходную величину. Параметры строгой модели подбираются так, чтобы выходная величина соответствовала предсказанию модели машинного обучения.
